...
function BottomNav() {
  ...
  const NAV = [
    { label: 'Home', path: '/', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#250000" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg> },
    ...
    { label: 'Favoritos', path: '/favoritos', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="#250000" stroke="#250000" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg> },
  ...
  return (
    <nav className="fixed bottom-0 z-50 flex items-center justify-around bg-[#eae1d4] border-t border-[#d6c8b3]"
      ...
      {NAV.map((item) => (
        <button key={item.label} onClick={() => navigate(item.path)}
          ...
          <span className="text-[9px] tracking-[0.1em]"
            style={{ fontFamily: "'DM Sans', sans-serif", color: item.path === '/favoritos' ? '#250000' : '#654a2b' }}>
            {item.label}
          </span>
        </button>
      ))}
  ...
  return (
    <div className="min-h-screen bg-[#eae1d4]">
      ...
      <main className="pt-14 px-4 pb-24">
        <h2 className="text-xs tracking-[0.2em] text-[#654a2b] uppercase mt-6 mb-4"
        ...
        {favoritos.length === 0 ? (
          <div className="flex flex-col items-center justify-center pt-20 gap-4">
            ...
            <p className="text-sm text-[#654a2b] text-center"
              style={{ fontFamily: "'DM Sans', sans-serif" }}>
            ...
            {favoritos.map((p) => (
              <div key={p.id} className="cursor-pointer" onClick={() => navigate(`/produto/${p.id}`)}>
                <div className="relative rounded-sm mb-2 overflow-hidden"
                  style={{ aspectRatio: '3/4', background: p.cor ?? '#d6c8b3' }}>
                  <button className="absolute top-2 right-2" aria-label="Remover favorito"
                    onClick={(e) => { e.stopPropagation(); toggle(p) }}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="#250000" stroke="#250000" strokeWidth="1.6">
                      <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
                    </svg>
                  </button>
                </div>
                <p className="text-sm leading-tight text-[#250000]"
