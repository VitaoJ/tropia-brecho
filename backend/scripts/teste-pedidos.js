...
r = await post('/pedidos', { cliente, endereco, itens: ['nao-e-uuid'] })
checar('id de peça inválido é recusado', r.status === 400)
r = await post('/pedidos', { cliente, endereco, itens: [] })
checar('carrinho vazio é recusado', r.status === 400)
r = await post('/pedidos', { cliente, endereco, itens: [a.id], forma_pagamento: 'boleto' })
checar('forma de pagamento desconhecida é recusada', r.status === 400)

// ── 2. cálculo no servidor ─────────────────────────────────────────
console.log('\n2. Conta fechada no servidor')
r = await post('/pedidos', { cliente, endereco, itens: [a.id, b.id], forma_pagamento: 'pix', total_esperado: 0.01 })
...
r = await post('/pedidos', { cliente, endereco, itens: [a.id, b.id], forma_pagamento: 'pix' })
...
r = await post('/pedidos', { cliente, endereco, itens: [a.id], forma_pagamento: 'credit_card' })
...
// cupom + pix acumulam
r = await post('/pedidos', { cliente, endereco, itens: [a.id, b.id], forma_pagamento: 'pix', cupom: 'teste20' })
...
r = await post('/pedidos', { cliente, endereco, itens: [a.id], cupom: 'NAOEXISTE' })
...
r = await post('/pedidos', { cliente, endereco, itens: [a.id] })
...
try {
  ...
  await c.query('UPDATE products SET sold = FALSE WHERE id = ANY($1::uuid[])', [[a.id, b.id]])
